...
if (isset ($_GET ['album'])) {
    ...
    foreach ($users as $user) {
        $images = $sql->getRows("SELECT album_images.sequence, album_images.location, album_images.title, users.usr FROM favorites LEFT JOIN album_images ON favorites.album = album_images.album AND favorites.image = album_images.id LEFT JOIN users ON favorites.user = users.id WHERE `favorites`.`album` = ? AND `favorites`.`user` = ?", [$album, $user['user']]);
