...
$row = $sql->getRow("SELECT email FROM users WHERE email='$email' AND id!={$systemUser->getId()}");
if ($row ['email']) {
...
$sql = "UPDATE users SET firstName='$firstName', lastName='$lastName', email='$email' WHERE id='" . $systemUser->getId() . "';";
...
if (isset ($_POST ['password']) && $_POST ['password'] != "") {
    ...
    $row = $sql->getRow("SELECT usr FROM users WHERE id='" . $systemUser->getId() . "' AND pass='$curPass'");
    if (!$row ['usr']) {
    ...
    $sql = "UPDATE users SET pass='$password' WHERE id='" . $systemUser->getId() . "';";
    mysqli_query($conn->db, $sql);
}
mysqli_query($conn->db, "INSERT INTO `user_logs` VALUES ( {$systemUser->getId ()}, CURRENT_TIMESTAMP, 'Updated User', NULL, NULL );");
