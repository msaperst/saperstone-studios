...
$sql = new Sql ();
$response = array();
foreach ($sql->getRows("SELECT DATE(usage.time) as date,COUNT(DISTINCT coalesce(usage.user,''),usage.ip ) AS count FROM `usage` LEFT JOIN `users` ON `usage`.`user` <=> `users`.`id` WHERE DATE(usage.time) > (CURDATE() + INTERVAL $start DAY) AND DATE(usage.time) <= (CURDATE() + INTERVAL " . ($start + $length) . " DAY) AND `usage`.`isRobot` = 0 $noAdmin GROUP BY DATE(usage.time);") as $r) {
    $date = explode('-', $r ['date']);
