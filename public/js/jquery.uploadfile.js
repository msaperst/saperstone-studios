...
(function ($) {
    if ($.fn.ajaxForm === undefined) {
        $.getScript("/js/jquery.form.min.js");
    }
    var feature = {};
    ...
    $.fn.uploadFile = function (options) {
        ...
        (function checkAjaxFormLoaded() {
            if ($.fn.ajaxForm) {
                ...
                $(s.uploadButtonLocation).prepend(uploadLabel);
                $(obj).append(obj.errorLog);

                if (s.showQueueDiv)
                    obj.container = $("#" + s.showQueueDiv);
                else
                    obj.container = $("<div class='ajax-file-upload-container'></div>").insertAfter($(s.uploadContainer));
            ...
            fileInput.change(function () {
                ...
                if (this.files) // support reading files
                {
                    ...
                    if (!isFileTypeAllowed(obj, s, filenameStr)) {
                        if (s.showError)
                            $("<div class='" + s.errorClass + "'><b>" + filenameStr + "</b> " + s.extErrorStr + s.allowedTypes + "</div>").appendTo(obj.errorLog);
                        return;
                    }
                ...
                if (s.serialize && feature.fileapi && feature.formdata) {
                    ...
                    for (var j = 0; j < fileArray.length; j++) {
                        if (s.showFileCounter)
                            fileList += obj.fileCounter + s.fileCounterStyle + fileArray[j] + "<br>";
                        else
                            fileList += fileArray[j] + "<br>";
                        obj.fileCounter++;
