...
class UpdateImageSharersTest extends TestCase {
    /**
     * @var Client
     */
    private $http;
    ...
    public function testNotLoggedIn() {
        try {
            $this->http->request('POST', 'api/update-image-sharers.php');
    ...
    public function testNoAlbum() {
        ...
        $response = $this->http->request('POST', 'api/update-image-sharers.php', [
            'cookies' => $cookieJar
        ]);
    ...
    public function testNoSharers() {
        ...
        $this->assertEquals(0, $this->sql->getRowCount("SELECT * FROM share_rights WHERE album = 999 AND image=999"));
    }

    /**
     * @throws GuzzleException
     */
    public function testSharers() {
        $cookieJar = CookieJar::fromArray([
            'hash' => '1d7505e7f434a7713e84ba399e937191'
        ], getenv('DB_HOST'));
        $response = $this->http->request('POST', 'api/update-image-sharers.php', [
        ...
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertEquals("", (string)$response->getBody());
        $users = $this->sql->getRows("SELECT * FROM share_rights WHERE album = 999 AND image=999");
     ...
     * @throws GuzzleException
     */
    public function testSharersAll() {
        $cookieJar = CookieJar::fromArray([
            'hash' => '1d7505e7f434a7713e84ba399e937191'
        ], getenv('DB_HOST'));
        $response = $this->http->request('POST', 'api/update-image-sharers.php', [
        ...
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertEquals("", (string)$response->getBody());
        $users = $this->sql->getRows("SELECT * FROM share_rights WHERE album = 999 AND image='*'");
