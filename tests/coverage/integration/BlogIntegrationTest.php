...
class BlogIntegrationTest extends TestCase {
    ...
    public function testGetId() {
        $blog = new Blog('999');
        $this->assertEquals(999, $blog->getId());
    }
    ...
    public function testAllDataLoadedMinimal() {
        ...
        $this->assertEquals(998, $blogInfo['id']);
    ...
    public function testAllDataLoaded() {
        ...
        $blog = new Blog(999);
        $blogInfo = $blog->getDataArray();
        $this->assertEquals(999, $blogInfo['id']);
        $this->assertEquals('Sample Blog', $blogInfo['title']);
        $this->assertNull($blogInfo['safe_title']);
        $this->assertEquals('January 1st, 2031', $blogInfo['date']);
        $this->assertEquals('', $blogInfo['preview']);
        ...
        $this->assertEquals(999, $blogInfo['content'][1][0]['blog']);
        ...
        $this->assertEquals(999, $blogInfo['content'][2][0]['blog']);
        ...
        $this->assertEquals(999, $blogInfo['comments'][0]['id']);
        $this->assertEquals(999, $blogInfo['comments'][0]['blog']);
        ...
        $this->assertEquals(998, $blogInfo['comments'][1]['id']);
        $this->assertEquals(999, $blogInfo['comments'][1]['blog']);
    ...
    public function testDeleteNoAccess() {
        ...
        $this->assertEquals(1, $this->sql->getRowCount("SELECT * FROM `blog_images` WHERE `blog_images`.`blog` = 999;"));
        $this->assertEquals(1, $this->sql->getRowCount("SELECT * FROM `blog_tags` WHERE `blog_tags`.`blog` = 999;"));
        $this->assertEquals(1, $this->sql->getRowCount("SELECT * FROM `blog_texts` WHERE `blog_texts`.`blog` = 999;"));
    ...
    public function testDelete() {
        ...
        $this->assertEquals(0, $this->sql->getRowCount("SELECT * FROM `blog_comments` WHERE `blog_comments`.`blog` = 999;"));
