...
class ApiUnitTest extends TestCase {
    ...
    public function testRetrieveGetIntNotSet() {
        $this->assertEquals( array('error' => 'Foo is required' ), $this->api->retrieveGetInt('bar', 'Foo', 'err') );
    }

    public function testRetrieveGetIntBlank() {
        $_GET['bar'] = "";
        $this->assertEquals( array('error' => 'Foo can not be blank' ), $this->api->retrieveGetInt('bar', 'Foo', 'err') );
        unset( $_GET );
    }
