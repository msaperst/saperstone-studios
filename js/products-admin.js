...
function saveSize() {
    $('.save-size-button').off().click(function(){
        ...
        $.post("/api/update-product-size.php", {
            ...
            $('i', button).addClass('fa-save').removeClass('fa-asterisk icon-spin');
    ...
    $('.add-size-button').off().click(function(){
        ...
        $.post("/api/add-product-size.php", {
            ...
            if( Math.round(data) == data && data !== '0' ) {
                ...
                var buttons = $('<td>' + 
                        '<button type="button" class="btn btn-xs btn-warning edit-size-button"><i class="fa fa-pencil-square-o"></i></button> ' + 
                        ...
                        '<button type="button" class="btn btn-xs btn-warning cancel-size-button hidden"><i class="fa fa-ban"></i></button> ' +
                '</td>');
                tr.append(buttons);
...
function addProduct() {
    $('.add-product-button').off().click(function(){
        ...
        BootstrapDialog.show({
            ...
            message: '<input id="product-name" class="form-control" />' + 
                    '<div id="product-error" class="error" />',
            buttons: [{
                    ...
                    $.post("/api/add-product.php", {
                        ...
                        if( Math.round(data) == data && data !== '0' ) {
                            ...
                            header.after( '<div class="col-md-4 col-sm-6 bootstrap-dialog" product-type="'+data+'">' + 
                                    ...
                                    '<button type="button" class="btn btn-xs btn-warning cancel-product-button hidden"><i class="fa fa-ban"></i></button>' +
                                    '<h3 product-type="'+data+'" class="text-center editable-header">'+$('#product-name').val()+'</h3>' +
                                    '<table class="table table-striped">' +
                                        '<thead>' +
                                            '<tr><th style="width:67px;"></th><th>Size</th><th>Cost</th><th>Price</th></tr>' +
                                        '</thead>' +
                                        '<tbody>' +
                                            '<tr>' +
                                                '<td>' +
                                                    '<button type="button" class="btn btn-xs btn-success add-size-button"><i class="fa fa-save"></i></button>' +
                                                '</td>' +
                                                ...
                                                '<td class="product-price"><input class="form-control input-sm" type="number" step="0.01" min="0"></td>' +
                                            '</tr>' +
                                        '</tbody>' +
                                    '</table>' +
