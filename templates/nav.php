...
<div id='displayed-alerts'
    ...
    foreach ($navSql->getRows($query) as $row) {
        if (!isset ($_COOKIE ["announcement-" . $row ['id']]) && Strings::startsWith($_SERVER['REQUEST_URI'], $row['path'])) {
            ...
            ?>
            <div
                    class="alert alert-warning<?php if ($row['dismissible']) {
                        echo " alert-dismissable fade in";
                    } ?>">
                <?php if ($row['dismissible']) {
                    echo "<a id='announcement-" . $row['id'] . "' href='#' class='close' data-dismiss='alert' aria-label='close' title='close'>×</a>";
                } ?>
                <strong><i class="fa fa-exclamation-triangle"
                           aria-hidden="true"></i></strong> <?php echo $row['message']; ?>
            </div>
            <?php
        }
    }
...
<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation"
    ...
    <div class="container">
        ...
        <div class="navbar-header">
            ...
            <a id="nav-logo-link-2" class="nav-logo-link navbar-brand" href="/">
                <img
                        class="nav-logo"
                        id="nav-logo-2"
                        src="/img/2014websitelogo250px.png"
