...
Production deployment is initiated by the DietPi web server. GitHub Actions builds the production images and publishes them to GitHub Container Registry (GHCR). The DietPi host periodically checks GitHub for the latest successful production-image workflow run and pulls the images itself.
...
Verify the timer using root privileges:

```bash
sudo systemctl status saperstone-deploy.timer
sudo systemctl list-timers saperstone-deploy.timer
```
