...
API, UI/Page, UI/Behat, and ZAP jobs all exercise the same full Docker Compose application. They use `bin/setup-ci-environment.sh` to create the common CI `.env`, prepare test folders, and disable the production Let's Encrypt certificate reference before launching the stack. Suite-specific setup remains in the individual workflow.
...
`.github/workflows/api-test.yml` runs for pull requests to `develop`. It uses the shared full-stack CI setup, builds and launches the application with Docker Compose, waits for the HTTP endpoint to become available, and runs `composer api-test`.
...
**Page Testing** uses the shared full-stack CI setup, starts ChromeDriver, and runs `composer ui-page-test`. JUnit, HTML, and custom report artifacts are retained by the workflow.
...
`.github/workflows/zap_scans.yml` defines OWASP ZAP baseline and full web application scans for pull requests and weekly scheduled execution. Each scan uses the same shared full-stack CI setup as the API and UI tests, waits for the application on port 90, and scans that running application.
