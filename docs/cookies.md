...
With explicit consent, Google Analytics, Meta Pixel, and the Facebook SDK may
create their own cookies. Their names and lifetimes are controlled by those
providers. Known analytics and social cookies are removed when the corresponding
consent is withdrawn. Native blog sharing uses the browser or device sharing
interface or clipboard and does not load a third-party sharing provider.
